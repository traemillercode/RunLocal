...
describe("roleOf", () => {
  it("maps guest / pending / verified me payloads to roles", () => {
    ...
    const pending: Me = { status: "signed_in", account: { id: "a", name: "N", email: "n@x.com", status: "pending", phase: "pending_review", badge: null, profilePhotoUrl: null } };
    const verified: Me = { status: "signed_in", account: { id: "b", name: "V", email: "v@x.com", status: "verified", phase: null, badge: "verified", profilePhotoUrl: null } };
...
describe("phase labels", () => {
  it("gives copy for each funnel stage", () => {
    expect(phaseLabel("phone")).toMatch(/phone/i);
