...
export function RecoveryPage({ sessionError }: { sessionError?: string }) {
  ...
  const submit=async()=>{setError(null);if(password.length<8){setError("Password must be at least 8 characters.");return}if(password!==confirm){setError("Passwords do not match.");return}setBusy(true);const r=await supabase.updatePassword(password);setBusy(false);if(!r.ok){setError(r.message);return}setDone(true)};
=======
