...
export function GroupDetailPage({ id }: { id: string }) {
  ...
  return <section className="mx-auto max-w-3xl px-4 py-6">
    ...
    <div className="mt-4 flex flex-wrap gap-3">{[group.websiteUrl, group.groupmeUrl, group.facebookUrl, group.instagramUrl].filter(Boolean).map((u, i) => <a className="text-[#FF5741] underline" href={u!} target="_blank" rel="noreferrer" key={u}>{["Website", "GroupMe", "Facebook", "Instagram"][i]}</a>)}</div>
  </section>;
}
