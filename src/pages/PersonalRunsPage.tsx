...
export function PersonalRunsPage() {
  ...
  if (me?.status !== "signed_in") return <Page><h1>Personal Runs</h1><p className="mt-2 text-slate-600">Sign in to keep a private calendar of your own runs.</p><Link className="mt-5 inline-flex rounded-full bg-[#14171C] px-5 py-3 font-semibold text-white" to="/login">Sign in</Link></Page>;
  if (role !== "verified") return <Page><h1>Personal Runs</h1><p className="mt-2 text-slate-600">Personal runs are available to verified runners.</p><Link className="mt-5 inline-flex rounded-full bg-[#14171C] px-5 py-3 font-semibold text-white" to="/verify">Verify account</Link></Page>;
