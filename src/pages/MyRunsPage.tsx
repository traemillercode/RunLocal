...
export function CalendarGrid({ upcoming, onRemove, removingId, onToggleKeep, keepingId, now = new Date() }: { upcoming: api.MyRunView[]; onRemove: (r: api.MyRunView) => void; removingId?: string | null; onToggleKeep?: (r: api.MyRunView) => void; keepingId?: string | null; now?: Date }) {
  ...
  return <div className="mt-6" role="group" aria-label="My Runs calendar"><div className="flex items-center gap-3"><div className="flex items-center gap-1"><button type="button" aria-label="Previous month" onClick={() => changeMonth(-1)} disabled={cursor <= monthKey(localISODate(now))} className="flex h-11 w-11 items-center justify-center rounded-xl bg-white text-lg font-bold text-slate-600 shadow-sm ring-1 ring-slate-200 hover:ring-slate-400 disabled:cursor-default disabled:text-slate-300 disabled:ring-slate-100">‹</button><button type="button" aria-label="Next month" onClick={() => changeMonth(1)} className="flex h-11 w-11 items-center justify-center rounded-xl bg-white text-lg font-bold text-slate-600 shadow-sm ring-1 ring-slate-200 hover:ring-slate-400">›</button></div><h2 aria-live="polite" className="text-sm font-extrabold uppercase tracking-wide text-slate-500">{monthLabel(cursor)}</h2></div><p className="mt-2 text-xs text-slate-400">Upcoming runs only — past history stays in the List view.</p><div className="mt-3 grid grid-cols-7 gap-1">{WEEKDAY_LABELS.map((label) => <div key={label} aria-hidden="true" className="pb-1 text-center text-[11px] font-extrabold uppercase tracking-wide text-slate-400">{label}</div>)}{days.map((cell, i) => cell.inMonth ? (() => { const count = runsByDate.get(cell.date)?.length ?? 0; const isToday = cell.date === today; const isActive = activeDate === cell.date; return <button key={cell.date} type="button" data-date={cell.date} onClick={() => setSelected(cell.date)} aria-pressed={isActive} aria-current={isToday ? "date" : undefined} aria-label={dayAriaLabel(cell.date, count)} tabIndex={cell.date === focusableDate ? 0 : -1} onKeyDown={(e) => handleDayKeyDown(e, cell.date)} className={`flex min-h-11 flex-col items-center justify-center rounded-xl text-sm font-bold shadow-sm ring-1 ring-slate-200 ${isActive ? "bg-[#14171C] text-white ring-[#14171C]" : isToday ? "bg-[#FF5741]/10 text-[#14171C] ring-2 ring-[#FF5741]" : "bg-white text-slate-700 hover:ring-slate-400"}`}><span>{cell.dayOfMonth}</span>{count > 0 ? <span aria-hidden="true" className={`mt-0.5 text-[11px] font-extrabold leading-none ${isActive ? "text-white/70" : "text-[#FF5741]"}`}>{count}</span> : null}{isToday ? <span className="sr-only">Today</span> : null}</button>; })() : <span key={`fill-${i}`} aria-hidden="true" className="min-h-11 rounded-xl" />)}</div><section aria-label="Runs on the selected day" className="mt-5">{activeDate ? <><h3 tabIndex={-1} data-day-panel-focus className="text-sm font-extrabold uppercase tracking-wide text-slate-500 focus:outline-none">{formatRunDate(activeDate)}{activeDate === today ? <span className="ml-2 rounded-full bg-[#FF5741]/15 px-2 py-0.5 text-[11px] font-extrabold uppercase text-[#14171C]">Today</span> : null}</h3>{dayRuns.length > 0 ? <div className="mt-3 space-y-3">{dayRuns.map((run) => <RunCard key={run.id} run={run} onRemove={onRemove} removing={removingId === run.id} upcoming onToggleKeep={onToggleKeep} keeping={keepingId === run.id} />)}</div> : <p tabIndex={-1} data-day-panel-focus className="mt-3 text-sm text-slate-500 focus:outline-none">No runs on this day.</p>}</> : <p tabIndex={-1} data-day-panel-focus className="mt-3 text-sm text-slate-500 focus:outline-none">No upcoming runs this month. Past runs you kept or checked in to stay in the List view.{nextRun ? ` Your next RSVP is in ${monthLabel(nextRun.date.slice(0, 7))}.` : ""}</p>}</section></div>;
}
