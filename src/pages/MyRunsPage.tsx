...
export function MyRunsPage() {
  ...
  return <Page><div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-start" data-tour-target="my-runs-header"><div><h1>My Runs</h1><p className="mt-2 text-sm text-slate-500">Your private RSVP list. Only you can see it.</p></div><div className="flex shrink-0 rounded-xl bg-slate-100 p-1" role="group" aria-label="My Runs view"><button type="button" aria-pressed={view === "list"} onClick={() => setView("list")} className={`min-h-11 rounded-lg px-3 py-2 text-xs font-bold ${view === "list" ? "bg-white shadow-sm" : "text-slate-500"}`}>List</button><button type="button" aria-pressed={view === "calendar"} onClick={() => setView("calendar")} className={`min-h-11 rounded-lg px-3 py-2 text-xs font-bold ${view === "calendar" ? "bg-white shadow-sm" : "text-slate-500"}`}>Calendar</button></div></div>{actionError ? <div role="alert" className="mt-4 rounded-xl bg-rose-50 p-3 text-sm font-medium text-rose-800 ring-1 ring-rose-200">{actionError}</div> : null}{!hasRuns ? <Empty /> : view === "calendar" ? <CalendarGrid upcoming={sections.upcoming} onRemove={remove} removingId={removingId} onToggleKeep={toggleKeep} keepingId={keepingId} /> : <div className="mt-6 space-y-8">{sections.upcoming.length > 0 && <RunSection title="Upcoming" runs={sections.upcoming} onRemove={remove} removingId={removingId} onToggleKeep={toggleKeep} keepingId={keepingId} upcoming />}{sections.past.length > 0 && <PastSection runs={sections.past} onRemove={remove} removingId={removingId} onToggleKeep={toggleKeep} keepingId={keepingId} />}</div>}</Page>;
