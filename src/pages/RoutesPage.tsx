        ...
        <div className="mt-5 grid gap-3">
          {routes.map((route) => (
            <Link
              key={route.id}
              to={`/routes/${route.id}`}
              className="flex items-center justify-between rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200/70 active:bg-slate-50"
            >
              <div>
                <p className="text-[15px] font-bold text-slate-900">{route.name}</p>
                <div className="mt-1 flex items-center gap-2">
                  <Chip tone="outline">{SURFACE_LABELS[route.surfaceType]}</Chip>
                  <span className="text-[13px] text-slate-500">{route.distanceMiles} mi · {route.elevationGainFt} ft gain</span>
                </div>
              </div>
