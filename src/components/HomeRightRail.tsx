...
export function HomeRightRail({ city }: { city: City }) {
  return <aside className="desktop-home-rail" aria-label="Local highlights">
    ...
    <section><p className="desktop-rail-kicker">Nearby groups</p>{city.groups.slice(0, 3).map((group) => <Link className="desktop-rail-item" to="/events" key={group.id}><b>{group.name}</b><span>{group.groupType === "rrca-chartered" ? "RRCA-chartered club" : "Community run group"}</span></Link>)}</section>
  </aside>;
}
