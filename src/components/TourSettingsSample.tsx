 ...
 */
export const TOUR_SETTINGS_SAMPLE_CAPTION = "A preview — your actual settings are on this page.";
...
/**
 * Presentational switch mirroring the Settings page's `role="switch"` row for
 * "Let people find me by name" (aria-checked, dark track, white knob).
 */
function SampleToggle({ label }: { label: string }) {
  return (
    <span
      role="switch"
      aria-checked={true}
      aria-label={`${label} on`}
      className="relative inline-flex h-6 w-10 shrink-0 rounded-full bg-[#14171C]"
    >
      <span aria-hidden="true" className="absolute left-[3px] top-[3px] h-[18px] w-[18px] rounded-full bg-white shadow" />
    </span>
  );
}
...
export function TourSettingsSample() {
  return (
    <div
      className="mt-4 overflow-hidden rounded-2xl bg-white ring-1 ring-slate-200"
    ...
    >
      ...
      <ul className="divide-y divide-slate-100">
        ...
        <li className="flex items-center justify-between gap-3 px-4 py-2.5">
          ...
          <SampleToggle label="Let people find me by name" />
        </li>
      </ul>
