...
function imageDimensions(bytes: Buffer, ext: string): { width: number; height: number } | null {
  if (ext === "png") {
    if (bytes.length < 24 || bytes.readUInt32BE(8) !== 13 || bytes.toString("ascii", 12, 16) !== "IHDR") return null;
    return { width: bytes.readUInt32BE(16), height: bytes.readUInt32BE(20) };
  }
  if (ext === "webp") {
    if (bytes.length < 30 || bytes.toString("ascii", 0, 4) !== "RIFF" || bytes.toString("ascii", 8, 12) !== "WEBP" || bytes.toString("ascii", 12, 16) !== "VP8X") return null;
...
function validateImageBytes(bytes: Buffer, ext: string, minEdge: number): string | null {
...
function decodeImage(dataUrl: string, minEdge = 64): { ok: true; bytes: Buffer; ext: string } | { ok: false; error: string } {
  ...
  const validation = validateImageBytes(bytes, ext, minEdge);
