...
export class GarminAdapter implements ProviderAdapter { provider:Provider="garmin"; attribution="Garmin"; configured=()=>false; authorizeUrl(_state:string):string{throw new Error("provider_not_configured");} async exchange(_code:string):Promise<{accessToken:string}>{throw new Error("provider_not_configured");} async revoke(){/* scaffold */} normalize(raw:unknown){return normalizeActivity("garmin",raw);} }
